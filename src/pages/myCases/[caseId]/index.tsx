...
export default function CaseViewPage() {
  ...
  const { searchKey, setSearchKey, currentViewingCase, addMemberToParty, prosecutorsAddressList, defendantsAddressList, loading: caseLoading, getStatusMessageAndStylesForCaseState, changeCaseState, declareCaseWinner } = useCase()
  ...
  const [caseWinner, setCaseWinner] = useState<boolean>();
  const [showCaseStateConfirmButton, setShowCaseStateConfirmButton] = useState<boolean>(false);
  const [showCaseWinnerConfirmButton, setShowCaseWinnerConfirmButton] = useState<boolean>(false);

  const [selectedProsecutorTab, setSelectedProsecutorTab] =
  ...
  useEffect(() => {
    if (currentViewingCase) {
      if (
        prosecutorsAddressList.length > 0 &&
        prosecutorsAddressList.includes(publicKey?.toBase58() as string)
      ) {
        setParty("prosecutor");
        return;
      }
      if (
        defendantsAddressList.length > 0 &&
        defendantsAddressList.includes(publicKey?.toBase58() as string)
      ) {
        setParty("defendant");
        return;
      }
      if (
        currentViewingCase.account.judge.toBase58() === publicKey?.toBase58()
      ) {
        if (!party) setParty("prosecutor");
        return;
      }
      if (
        defendantsAddressList.length > 0 &&
    ...
    if (currentViewingCase && party) {
      const fetchRecords = async () => {
        const res = await fetch(
          `/api/appwrite/storage/documents?caseId=${currentViewingCase.publicKey.toBase58()}&party=${party}`,
          {
            method: "GET",
            headers: {
        ...
        let arweaveFiles = await getAllRecordsFromArweave(arweaveDocumentList)
  ...
  }, [hasUploadedDocument, party, currentViewingCase]);

  const emptyDocumentListStyles = "h-2/5 gap-12 mt-4 p-2 w-3/4";
  const nonEmptyDocumentListStyles =
    "h-2/5 gap-12 mt-4 p-2 w-3/4 grid grid-cols-4 col-span-4";
  ...
  const handleCaseStateChange = async () => {
    try {
      await changeCaseState(new PublicKey(router.query.caseId as string), {
        [`${caseState}`]: {},
      } as CaseState);
      setShowCaseStateConfirmButton(false);
    } catch (err) { }
  ...
  const handleAddMemberToParty = async () => {
    if (!data || data.length !== 44) {
      ...
      await addMemberToParty(
        currentViewingCase?.publicKey as PublicKey,
        new PublicKey(data),
        party as PartyType
      );
    }
  ...
  const hasInputWalletAddress = useMemo(
    (): boolean => (data && data.length === 44 ? true : false),
    [data]
  );
      ...
      <div className="flex flex-col">
        <div className="flex flex-row w-full">
          <Modal id="AddMembersToCase">
            <form
              className="mx-auto space-y-8 flex flex-col justify-center text-white"
              onSubmit={(e) => {
                e.preventDefault();
              }}
            >
              <h1 className="text-center text-xl">Add members to case</h1>
              ...
              <h1 className="font-bold text-center text-xl">OR</h1>
              <Button
                state={hasInputWalletAddress ? "disabled" : "initial"}
                onClick={() => {
                  setData("No result");
                }}
                className="self-center hover:underline bg-green-700 hover:bg-green-8000 text-white w-1/3"
              >
                Scan QR
              </Button>
              {data === "No result" && (
              ...
              <Button
                state={!hasInputWalletAddress ? "disabled" : "initial"}
                onClick={handleAddMemberToParty}
                className="self-center hover:underline bg-green-700 hover:bg-green-8000 text-white w-1/3"
              >
                Confirm
              </Button>
          ...
          <Modal id="FileUploadModal">
            ...
            >
              <h1 className="text-center text-xl">Upload your document</h1>
              <input
              ...
              />
              <Button
                state={!uploadedFile ? "disabled" : "initial"}
          ...
          <div className="flex mx-4 mt-4 flex-row text-white  justify-between w-full">
              ...
              <div className="text-4xl">
                ...
                {user.typeOfUser.judge ? (
                  <>
                    <select
                      className={
                        "ml-4 rounded-xl select select-bordered select-md"
                      }
                      onChange={(e) => {
                        setCaseState(e.target.value as CaseState);
                        setShowCaseStateConfirmButton(true);
                      }}
                    >
                      {Object.values(CaseStatePairs).map(
                        ({ message: CaseStateTag, classNames }, index) => (
                          <option
                            value={Object.keys(CaseStatePairs)[index]}
                            className={
                              classNames + " my-4 text-lg font-light rounded-xl"
                            }
                            selected={
                              getStatusMessageAndStylesForCaseState(
                                currentViewingCase.account.caseState
                              ).message === CaseStateTag
                            }
                            disabled={
                              getStatusMessageAndStylesForCaseState(
                                currentViewingCase.account.caseState
                    ...
                    {showCaseStateConfirmButton && (
                      <Button
                        state="initial"
                        onClick={handleCaseStateChange}
                        className="hover:underline bg-success hover:bg-success text-white ml-4"
                      >
                        ✓
                      </Button>
                    )}
                  ...
                  <span
                    className={
                      ...
                      " text-lg opacity-90 ml-4 border border-fuchsia-600 rounded-xl p-2"
                    }
                  >
                    {
                      getStatusMessageAndStylesForCaseState(
            ...
            <Button
              ...
              className="hover:underline bg-fuchsia-400 hover:bg-fuchsia-600 text-black"
            >
              Upload documents +
            </Button>
          </div>
        ...
        <div className="flex flex-row w-3/4 mx-4">
          {user.typeOfUser.judge &&
            ...
            <div
              role="tablist"
              className="tabs tabs-boxed bg-[#0B0708] border-white border w-1/2 mx-auto my-4 font-bold"
            >
              <button
                role="tab"
                className={
                  selectedProsecutorTab ? activeTabStyles : inactiveTabStyles
                }
                onClick={() => {
                  setSelectedProsecutorTab(true);
                  setParty("prosecutor");
                }}
              >
                Prosecutor
              ...
              <button
                ...
                onClick={() => {
                  setSelectedProsecutorTab(false);
                  setParty("defendant");
                }}
              >
                Defendant
        ...
        <div className="flex flex-row">
          {
            caseLoading ?
              ...
              <div className={hasNoDocuments ? emptyDocumentListStyles : nonEmptyDocumentListStyles}>
                {
                  !hasNoDocuments ?
                    documents.map((file: ArweaveFile | FileMetadata) => <FileCardComponent key={file.name} caseId={router.query.caseId as string} fileName={file.name} fileId={(typeof file === 'object' && file.source === "appwrite" && '$id' in file) ? file["$id"] : file.txnId as string} fileHref={file.href} fileSource={file.source} fileMimeType={file.mimeType} />)
                    :
          ...
          <div className="flex mt-4 flex-col ml-6 gap-y-6 border border-white p-8 rounded-xl">
            ...
            </ul>
            {
              user.typeOfUser.judge &&
              <>
                <select className="rounded-xl select select-secondary select-bordered select-md text-white" onChange={(e) => { setCaseWinner((e.target.value as string === "true") as boolean); setShowCaseWinnerConfirmButton(true); }}>
                  <option value={"none"} className="my-4 text-md font-light rounded-xl" selected={currentViewingCase?.account.caseWinner ? false : true} disabled={currentViewingCase?.account.caseWinner ? false : true}>No winner yet</option>
                  <option value={"true"} className="my-4 text-md font-light rounded-xl" selected={currentViewingCase?.account.caseWinner?.prosecutor ? true : false} disabled={currentViewingCase?.account.caseWinner?.prosecutor ? true : false}>Prosecutor</option>
                  <option value={"false"} className="my-4 text-md font-light rounded-xl" selected={currentViewingCase?.account.caseWinner?.defendant ? true : false} disabled={currentViewingCase?.account.caseWinner?.defendant ? true : false}>Defendant</option>
                </select>
                {
