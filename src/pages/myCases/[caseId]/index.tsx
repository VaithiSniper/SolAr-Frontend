...
export default function CaseViewPage() {
  ...
  const { handleUploadToArweave, setFileBufferVal, getAllRecordsFromArweave } =
    useArweave();
  const {
    searchKey,
    setSearchKey,
  ...
  const [showCaseStateConfirmButton, setShowCaseStateConfirmButton] =
  ...
  useEffect(() => {
    if (currentViewingCase && party) {
      const fetchRecords = async () => {
        ...
        const arweaveDocumentList: [string?] =
          currentViewingCase.account[`${party}`].documents;
  ...
  const handleFileUpload = async (e: FormEvent) => {
    ...
    if (user.typeOfUser.client) {
      await handleUploadToAppwrite(
        router.query.caseId as string,
        party as "prosecutor" | "defendant"
      );
        ...
        <div className="flex flex-row">
          {caseLoading ? (
            ...
            <div
              className={
                hasNoDocuments
                  ? emptyDocumentListStyles
                  : nonEmptyDocumentListStyles
              }
            >
              {!hasNoDocuments ? (
                documents.map((file: ArweaveFile | FileMetadata) => (
                  <FileCardComponent
                    key={file.name}
                    caseId={router.query.caseId as string}
                    fileName={file.name}
                    fileId={
                      typeof file === "object" &&
                      file.source === "appwrite" &&
                      "$id" in file
                        ? file["$id"]
                        : (file.href as string)
                    }
          ...
          <div className="flex mt-4 flex-col ml-6 gap-y-6 border border-white p-8 rounded-xl">
            ...
            {user.typeOfUser.judge && (
              <Button
                state="initial"
                onClick={() => {
                  document.getElementById("AddMembersToCase").showModal();
                }}
                className="hover:underline bg-fuchsia-400 hover:bg-fuchsia-600 text-black"
              >
                Add {party}
            ...
            <Button
              state="initial"
              onClick={() => {
                document.getElementById("ViewMembersModal").showModal();
              }}
              className="hover:underline bg-fuchsia-400 hover:bg-fuchsia-600 text-black"
            >
              View members
