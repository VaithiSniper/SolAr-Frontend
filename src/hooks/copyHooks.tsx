...
export function useCase() {
  ...
  const addEventToCase = (caseAddress: PublicKey, event: CaseEvent) => {
    const updatedCases = cases.map((caseItem) => {
      if (caseItem.publicKey.equals(caseAddress)) {
        const updatedEvents = [...caseItem.account.events, event];
        return {
          ...caseItem,
          account: {
            ...caseItem.account,
  ...
  const addMemberToParty = async (caseAddress: PublicKey, memberAddress: PublicKey, partyType: PartyType) => {
    if (program && publicKey) {
      try {
        ...
        addEventToCase(caseAddress, {
          type: CaseEventType.MemberAdded,
          message: `${memberPda} was added to ${partyType} party`,
          classNames: "step step-success"
        });
      } catch (err: any) {
        toast.error(err.toString())
